...
%Acrotex
\makeatletter
\def\eq@drivername{3} %Kuben -chybí v eforms.sty, volba pdftex tento pøíkaz nenadefinuje. Jaká hodnota je vhodná?
\makeatother
\usepackage[czech]{exerquiz}
\usepackage[ImplMulti]{dljslib}
\useBeginQuizButton
\useEndQuizButton
\PTsHook{(\eqPTs\text{b.})}           % poèet bodù u otázky
\negPointsAllowed                     % umožòuje záporný souèet bodù, i záporné procenta.
...
\everyBeginQuizButton{\BC{\BeginQuizButtonColor}\CA{Zacátek testu}}  %Diakritiku doplnil Kuben; proè nìkdy nefunguje è?
...
\def\vyhodnoceni#1{%
    ...
    \noindent%
    \begin{tabular}{ll}%
        \noindent Správnì zodpovìzené otázky:& \ScoreField{#1}\\%
        \noindent Získané body:& \PointsField{#1}\\%
        \noindent Procento úspìšnosti:& \PercentField{#1}%
    \end{tabular}\newline%
    \eqButton[\CA{Výsledky}]{#1}%
    \medskip}
...
\def\AnswerFieldColor    {0.8 0.8 0.95}   %Jako pozadí panelu, aby nebyl zpoèátku vidìt. Kuben
...
\usepackage{vlna} %Musí být za naèítáním exerquiz.djs, havaruje. Kuben
...
%\vspace{25mm}\par
\vfill\par
\mbox{\def\RBW{.8\panelwidth}\csname math@\thepage\endcsname} %Políèko, kde se zobrazí výsledky úloh
\vfill\par
...
\vss
   \Acrobatmenu{FullScreen}%
      ...
      {Celá obrazovka$\big/$Okno}}%
\iffalse   % V AR 9 nefunguje
   \vspace{3mm}\par
   {\rmfamily\upshape\mdseries\normalsize V oknì:}\\[4pt]%Kuben
   \Acrobatmenu{ShowHideToolBar}%
      {\addButton{.9\panelwidth}%
      {Zobrazit$\big/$Skrýt ikony}}\\[2pt]
   \Acrobatmenu{ShowHideMenuBar}%
      {\addButton{.9\panelwidth}%
      {Zobrazit$\big/$Skrýt menu}}%
   \vspace*{3mm}%
\fi
\vspace*{10mm}
}}}%
\makeatother%
%% Konec definice ovládacího panelu
...
%    \def\kolik{30}%                                     %doplnìno, aby se vešlo o øádek víc
...
\addto\captionsczech{\renewcommand{\figurename}{Obr.}\renewcommand{\tablename}{Tab.}\renewcommand{\proofname}{Dùkaz}}
...
%Babel pøedefinuje \contentsname z pdfscreen - Kuben
\makeatletter
\AtBeginDocument{%
